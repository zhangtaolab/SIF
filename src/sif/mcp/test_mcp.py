...
async def test_stdio_mode():
    ...
    notification = JsonRpcNotification(method="notifications/initialized", params={})
    await server.handle_notification(notification)
    print("   Done")

    # 3. List tools
    print("\n3. Listing tools...")
    request = JsonRpcRequest(id=2, method="tools/list", params={})
    response = await server.handle_request(request)
    ...
    request = JsonRpcRequest(
        id=3,
        method="tools/call",
        params={"name": "query", "arguments": {"query": "machine learning", "limit": 3}},
    )
    response = await server.handle_request(request)
    ...
    request = JsonRpcRequest(id=4, method="tools/call", params={"name": "status", "arguments": {}})
    response = await server.handle_request(request)
    content = response.result["content"][0]["text"]
    result = json.loads(content)
