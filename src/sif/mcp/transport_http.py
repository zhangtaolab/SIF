...
def create_http_app(
    ...
    _http_transport = HTTPTransport(server=server, config=config, cors_origins=cors_origins)
...
def main():
    ...
    asyncio.run(
        run_http_server(
            host=args.host, port=args.port, cors_origins=args.cors_origins, log_level=args.log_level
        )
    )
