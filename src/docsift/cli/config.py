...
class Config:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "collections": {
                name: asdict(coll) for name, coll in self.collections.items()
            },
            "contexts": {
                path: asdict(ctx) for path, ctx in self.contexts.items()
            },
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "Config":
        ...
        if "contexts" in data:
            config.contexts = {
                path: ContextConfig(**ctx_data)
                for path, ctx_data in data["contexts"].items()
            }
    ...
    def add_collection(self, name: str, path: str, mask: Optional[str] = None) -> None:
        ...
        self.collections[name] = CollectionConfig(
            name=name, path=path, mask=mask
        )
