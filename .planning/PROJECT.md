...
- [ ] 修复现有 bug 和占位实现（stub）：硬编码模型路径、随机 embedding fallback、MCP mock 工具等
...
- [ ] 依赖声明完整性：补全 `pyproject.toml` 中缺失的运行时依赖
...
|----------|-----------|---------|
| 完全复刻 qmd 功能 | 用户明确要求功能对等 | — Pending |
| 本地模型 + API 切换双支持 | 提升灵活性和可访问性 | — Pending |
| 优先修复 bug/stub，再补 CLI，再做 rerank/MCP | 先让现有代码可用，再扩展功能 | — Pending |
